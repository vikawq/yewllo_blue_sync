...
对同一个 DAG 的所有候选，把 throughput 归一到 `[0,1]`。设 program `P` 的真实归一 throughput 为 `y`，最内层 statements 集为 `S(P)`。预测为：
...
其中 `w_i` 是该 subgraph 在模型中出现次数。Ansor 以历史改善率和相似 task 的性能作 optimistic estimate，近似 `∂f/∂t_i`，每轮把资源给绝对梯度最大的 task，并用 ε-greedy 保留探索。
