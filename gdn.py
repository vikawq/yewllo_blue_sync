...
from vllm_ascend._310p.ops.causal_conv1d import (
    causal_conv1d_fn as causal_conv1d_fn_pytorch,
    causal_conv1d_update as causal_conv1d_update_pytorch,
)
...
def _host_ints_to_device_tensor(values, device: torch.device) -> torch.Tensor | None:
    if values is None:
        return None
...
def _causal_conv1d_activation(activation_mode: int) -> str | None:
    return "silu" if activation_mode else None


def _npu_causal_conv1d_custom_or_fallback(
    output: torch.Tensor,
    x: torch.Tensor,
    weight: torch.Tensor,
    *,
    ...
    fallback_query_start_loc_opt=None,
) -> torch.Tensor:
    try:
    ...
    weight_for_fallback = weight.transpose(0, 1).contiguous()
    conv_state_for_fallback = conv_state.transpose(-1, -2)
    activation = _causal_conv1d_activation(activation_mode)
    fallback_query_start_loc = fallback_query_start_loc_opt
    if fallback_query_start_loc is None:
        fallback_query_start_loc = query_start_loc_opt
    cache_indices = _host_ints_to_device_tensor(cache_indices_opt, x.device)

    if run_mode == 0:
        query_start_loc = _normalize_prefill_query_start_loc(
            fallback_query_start_loc,
            cache_indices,
            x.shape[0],
            x.device,
        )
        initial_state_mode = _host_ints_to_device_tensor(initial_state_mode_opt, x.device)
        fallback_out = causal_conv1d_fn_pytorch(
            x.transpose(0, 1),
            weight_for_fallback,
            bias=bias_opt,
            activation=activation,
            conv_states=conv_state_for_fallback,
        ...
        query_start_loc = _host_ints_to_device_tensor(fallback_query_start_loc, x.device)
        num_accepted_tokens = _host_ints_to_device_tensor(num_accepted_tokens_opt, x.device)
        fallback_out = causal_conv1d_update_pytorch(
            x,
            conv_state_for_fallback,
            weight_for_fallback,
            bias=bias_opt,
            activation=activation,
...
def _chunk_gated_delta_rule_or_fallback(
    *,
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    try:
        ...
        if not _should_fallback_chunk_gated_delta_rule(exc):
            raise
        return chunk_gated_delta_rule_pytorch(
            q=q,
            k=k,
...
def _npu_recurrent_gated_delta_rule_or_fallback(
    *,
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    ...
    fallback_state_indices: torch.Tensor | None = None,
    ...
    try:
        return torch.ops._C_ascend.npu_recurrent_gated_delta_rule(
            ...
            g=g.squeeze(0) if g is not None else g,
            beta=beta.squeeze(0) if beta is not None else beta,
            state=state,
            scale=scale,
        ...
        if not _should_fallback_recurrent_gated_delta_rule(exc):
            raise
    ...
    fallback_out, fallback_state = fused_recurrent_gated_delta_rule_pytorch(
        ...
        cu_seqlens=cu_seqlens,
        ssm_state_indices=fallback_state_indices if fallback_state_indices is not None else ssm_state_indices,
        num_accepted_tokens=num_accepted_tokens,
        use_qk_l2norm_in_kernel=use_qk_l2norm_in_kernel,
    )
    state.copy_(fallback_state.transpose(-1, -2).contiguous().to(state.dtype))
    return fallback_out
    ...
    def _forward_core(
            ...
            if mixed_qkv_non_spec is not None:
                if get_pcp_group().world_size > 1:
                    ...
                    _npu_causal_conv1d_custom_or_fallback(
                        ...
                        pad_slot_id=PAD_SLOT_ID,
                        run_mode=0,
                        fallback_query_start_loc_opt=non_spec_query_start_loc,
                    )
        ...
        if spec_sequence_masks is not None:
            ...
            core_attn_out_spec = _npu_recurrent_gated_delta_rule_or_fallback(
                ...
                fallback_state_indices=spec_state_indices_tensor,
        ...
        if attn_metadata.num_prefills > 0:
            ...
            (core_attn_out_non_spec, last_recurrent_state) = _chunk_gated_delta_rule_or_fallback(
                q=query_non_spec,
                k=key_non_spec,
                v=value_non_spec,
                g=g_non_spec,
