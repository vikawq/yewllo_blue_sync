...
def _clear_ssm_states_pytorch(ssm_states: torch.Tensor, has_initial_state: torch.Tensor) -> None:
...
def clear_ssm_states(ssm_states: torch.Tensor, has_initial_state: torch.Tensor) -> None:
    ...
    try:
        ...
        msg = str(exc)
        if "function' object is not subscriptable" not in msg and "Device properties not initialized" not in msg:
            raise
        _clear_ssm_states_pytorch(ssm_states, has_initial_state)
